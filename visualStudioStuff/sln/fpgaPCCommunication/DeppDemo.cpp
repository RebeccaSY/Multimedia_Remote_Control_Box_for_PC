...
void DoGetReg() {
	...
	while (true) {
		// For socket programming
		WORD sock_vers = MAKEWORD(2,2);
		WSADATA data;
		const char opt = 1;
		if (WSAStartup(sock_vers, &data) != 0) {
			std::cout << "something happened 1\n";
			return;
		}
		...
		if (setsockopt(sclient, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt))) {
			std::cout << "sockopt failed\n";
			return;
		}
		// set server side
		...
		inet_pton(AF_INET, "192.168.43.249", &serv_addr.sin_addr.s_addr);
		...
		inet_pton(AF_INET, "192.168.43.82", &client_addr.sin_addr.s_addr);
		...
		if (connect(sclient, (sockaddr*)&serv_addr, sizeof(serv_addr)) == SOCKET_ERROR) {
        ...
        if (idData == NULL) {
            std::cout << "ignoring data and sending 0\n";
			char data_send[1];
			sprintf(data_send, "%d", 0);
			int send_code = send(sclient, data_send, strlen(data_send), 0);
			// sleeps for 500ms
			Sleep(500);
			closesocket(sclient);
			WSACleanup();
			...
			char data_send[1];
			sprintf(data_send, "%d", idData);
	        // send to python port
			std::cout << "before socket stuff\n";
			// const char *data_send = "hello, there";
			std::cout << "after casting stuff\n";
			std::cout << "before sendto sending: " << data_send << "\n";
	        int send_no = send(sclient, data_send, strlen(data_send), 0);
	        std::cout << "after sendto with send_no " << send_no << " and  sending data: " << data_send << "\n";
	        // sleeps for 500ms
	        Sleep(500);
	        closesocket(sclient);
	        WSACleanup();
